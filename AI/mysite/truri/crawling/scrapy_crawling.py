...
class NaverSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        # 카페 iframe
        if "blog" not in response.url:
            iframe = response.css('#cafe_main').extract()
            yield scrapy.Request(url="https:" + iframe[0], callback=self.parse_iframe_cafe)
        # 블로그 iframe
        else:
            iframe = response.css('#mainFrame::attr(src)').extract()
            yield scrapy.Request(url="https://blog.naver.com/" + iframe[0], callback=self.parse_iframe_blog)

    def parse_iframe_blog(self, response):
    ...
    def parse_iframe_cafe(self, response):
        print("Cafe : in iframe")
        image = response.css(".se-image-resource").xpath("@src").extract()
        ...
        # 본문 크롤링
        content_data = response.css(".se-main-container").xpath(".//p//text()").getall()
        print(content_data)
        one_content = ""
        for line in content_data:
            if line != "\u200b":
...
def scrapy_crawling(query, page) :
    ...
    # 각각 다른 탭으로 검색했을 때, 블로그는 30개, 카페는 10개 단위
    # 네이버 블로그 검색결과 url
    ...
    # 네이버 블로그, 카페 검색결과 url
    url.append("https://s.search.naver.com/p/review/search.naver?rev=44&where=view&api_type=0&start="+ str(naver_page) +"&query=" + query +
               "&nso=&nqx_theme=%7B%22theme%22%3A%7B%22main%22%3A%7B%22name%22%3A%22restaurant_list%22%7D%7D%7D&main_q="+
               "&mode=normal&q_material=&ac=1&aq=0&spq=0&st_coll=&topic_r_cat=&nx_search_query=&nx_and_query="+
               "&nx_sub_query=&prank=31&sm=tab_jum&ssc=tab.view.view&ngn_country=KR&lgl_rcode=02135103"+
               "&fgn_region=&fgn_city=&lgl_lat=37.3614503&lgl_long=127.1115333&_callback=viewMoreContents")
